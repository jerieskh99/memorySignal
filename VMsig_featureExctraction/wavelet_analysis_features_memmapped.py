...
def access_shared_memory(shm_name, shm_shape, shm_dtype):
    # Access the shared memory
    shm = shared_memory.SharedMemory(name=shm_name)
    full_signal = np.ndarray(shm_shape, dtype=shm_dtype, buffer=shm.buf)
    return shm, full_signal
...
def process_1d_wavelet(row_initial, shm_name, shm_shape, shm_dtype, output_data_plot_dirs, wavelet, level=2, is_complex=False, is_visualize=False):
    """
    ...
    is_complex (bool): If True, handles the data as complex; otherwise processes real and imaginary parts separately.
    """
    shm, combined_data = access_shared_memory(shm_name=shm_name, shm_shape=shm_shape, shm_dtype=shm_dtype)
...
# Process a batch of signals/pages over time wavelet.
def process_1d_wavelet_time_segment(t_div, row_initial, shm_name, shm_shape, shm_dtype, output_data_plot_dirs, wavelet='db4', level=2, is_complex=False, is_visualize=False):
    """
    Processes each row (corresponding to a signal/page) in the combined data using 1D Wavelet Transform, visualizes and saves the coefficients, and generates scalograms.
...
def flatten_feature_map_1d(feature_map, parent_key=""):
    if feature_map is None:
        raise ValueError("The input feature_map is None. Please provide a valid dictionary.")
    ...
    for key, value in feature_map.items():
        # Construct the new key with the parent key for flattening
        new_key = f"{parent_key}_{key}" if parent_key else key

        # Check if the value is a dictionary (nested structure)
        if isinstance(value, dict):
            # Recursive call for nested dictionaries
            flattened.update(flatten_feature_map_1d(value, new_key))
        
        # Handle lists (for example, skewness and kurtosis lists)
        elif isinstance(value, list):
            for i, item in enumerate(value):
                item_key = f"{new_key}_{i}"
                if isinstance(item, dict):
                    flattened.update(flatten_feature_map_1d(item, item_key))
                elif isinstance(item, complex):
                    flattened[f"{item_key}_real"] = item.real
                    flattened[f"{item_key}_imag"] = item.imag
                else:
                    flattened[item_key] = item

        # Check if the value is complex and separate real and imaginary parts
        elif isinstance(value, complex):
            flattened[f"{new_key}_real"] = value.real
            flattened[f"{new_key}_imag"] = value.imag
        ...
        # Direct assignment for other types (int, float, etc.)
        else:
            flattened[new_key] = value
...
def flatten_feature_map_1d_dep(feature_map):
    if feature_map is None:
        raise ValueError("The input feature_map is None. Please provide a valid dictionary.")

    flattened = []

    def aux_flatten(value):
        """Recursively flatten dictionaries, lists, arrays, and complex numbers."""
        flattened = []

        if isinstance(value, dict):
            # Recursively flatten each item in the dictionary
            for v in value.values():
                aux_flatten(v)
    
        elif isinstance(value, complex):
            # Add real and imaginary parts separately
            flattened.extend([value.real, value.imag]) # Convert array to list and extend
        
        elif isinstance(value, (list, np.ndarray)):
            # Recursively flatten each element in the list or array
            for item in value:
                aux_flatten(item)

        elif isinstance(value, (np.float64, np.complex128)):
            flattened.append(value.item()) # Convert to Python float and append

        else:
            flattened.append(value)  # Directly append other scalar value

    # Start the recursive flattening from the root feature_map
    aux_flatten(feature_map)
                
    return np.array(flattened)  # Return as a 1D NumPy array for consistency
...
#     with h5py.File(hdf5_file, 'w') as h5file:
#         # Create datasets from the flattened dictionary
#         for key, value in flattened.item():
#             h5file.create_dataset(key, data=value)
    
#     print(f"HDF5 file saved to {hdf5_file}")

def create_hdf5(flattened, features_dir):
    if not isinstance(flattened, np.ndarray):
        raise ValueError("The input 'flattened' should be a 1D NumPy array.")
    
    hdf5_file = os.path.join(features_dir, 'features.h5')

    with h5py.File(hdf5_file, 'w') as h5file:
        h5file.create_dataset('flattened_features', data=flattened)
...
def create_files(features, features_dir):
    ...
    if features is not None:
        print("create_files features ok")
    else:
        print("create_files features are None")
    print("-----------------------------------------------------------------------")
    #Creates features files
    flattened_json = flatten_feature_map(features)
    # flattened = flatten_feature_map_1d(features)
    print("create_files flattened")
    print("-----------------------------------------------------------------------")
    ...
    create_json(flattened_json, features_dir)
...
def create_scalogram_1d(coeffs, levels, block_index, signal_plot_dir):
    """
    Creates and saves a DWT-based scalogram for a 2D block with complex coefficients.

    Parameters:
    coeffs (tuple): Tuple of real and imaginary coefficients from the DWT (coeffs_real, coeffs_imag).
    levels (int): Number of decomposition levels.
    block_index (int): The index of the current block being processed.
    signal_plot_dir (str): Directory to save the output scalogram plot.
    """
    print(f"Creating DWT-based scalogram for block {block_index} with complex coefficients")

    # Unpack the real and imaginary coefficients
    coeffs_real, coeffs_imag = coeffs

    # Compute magnitude for each level
    magnitudes = []
    max_width = 0
    for i, (real, imag) in enumerate(zip(coeffs_real, coeffs_imag)):
        if i == 0:
            # Approximation coefficients (low-frequency)
            magnitude = np.sqrt(np.abs(real) ** 2 + np.abs(imag) ** 2)
        else:
            # Detail coefficients (horizontal, vertical, diagonal)
            cH_real, cV_real, cD_real = real
            cH_imag, cV_imag, cD_imag = imag

            # Compute magnitude for detail coefficients
            magnitude = np.sqrt(
                np.abs(cH_real) ** 2 + np.abs(cH_imag) ** 2 +
                np.abs(cV_real) ** 2 + np.abs(cV_imag) ** 2 +
                np.abs(cD_real) ** 2 + np.abs(cD_imag) ** 2
            )
        magnitudes.append(magnitude)
        max_width = max(max_width, magnitude.shape[1])  # Track the maximum width

    # Pad all magnitudes to the same width
    aligned_magnitudes = []
    for mag in magnitudes:
        padded_mag = np.zeros((mag.shape[0], max_width))  # Create a zero-padded array
        padded_mag[:, :mag.shape[1]] = mag  # Copy the original data
        aligned_magnitudes.append(np.mean(padded_mag, axis=0))

    # Stack aligned magnitudes for visualization
    energy_matrix = np.vstack(aligned_magnitudes)

    # Plot scalogram
    plt.figure(figsize=(12, 8))
    plt.imshow(
        energy_matrix,
        extent=[0, energy_matrix.shape[1], levels, 0],
        cmap='viridis',
        aspect='auto',
    )
    plt.title(f'DWT Scalogram - Block {block_index}')
    plt.xlabel('Pages (Index)')
    plt.ylabel('Decomposition Levels')
    plt.colorbar(label='Energy (Magnitude)')
    
    # Save the scalogram
    if not os.path.exists(signal_plot_dir):
        os.makedirs(signal_plot_dir)

    scalogram_path = os.path.join(signal_plot_dir, f'dwt_scalogram_block_{block_index}.png')
    plt.savefig(scalogram_path)
    plt.close()

    print(f"DWT Scalogram for Block {block_index} saved to {scalogram_path}")
...
#     signal (np.ndarray): The 1D data array (signal).
#     signal_index (int): The index of the current signal being processed.
#     signal_plot_dir (str): Directory to save the output scalogram plots.
#     """
#     print(f"Creating scalogram for signal {signal_index}")

#     # Validate signal
#     if len(signal.shape) != 1:
#         raise ValueError(f"Expected a 1D signal, but got shape {signal.shape}")
...
#     sampling_frequency = 1  # Assuming normalized time index; adjust if actual sampling rate is known

#     # Perform CWT
#     coefficients, frequencies = pywt.cwt(signal, scales, wavelet_name, sampling_period=1 / sampling_frequency)

#     # Generate the scalogram plot
#     plt.figure(figsize=(10, 6))
#     plt.imshow(
#         np.abs(coefficients),
...
#     # Save the scalogram figure
#     scalogram_path = os.path.join(signal_plot_dir, f'scalogram_signal_{signal_index}.png')
#     plt.savefig(scalogram_path)
#     plt.close()

#     print(f'Scalogram for Signal {signal_index} processed and saved to {scalogram_path}')
...
def process_block_2d_wavelet_features_single_block(shm_name, shm_shape, shm_dtype, row_initial, n, batch_index, t_start, t_end, num_pages, 
                                                   output_data_plot_dirs, wavelet, level, is_visualize, is_complex):
    
    shm, full_signal = access_shared_memory(shm_name=shm_name, shm_shape=shm_shape, shm_dtype=shm_dtype)
...
def process_block_2d_wavelet_features_parallel(shm_name, shm_shape, shm_dtype, row_initial, n, num_batches, output_data_plot_dirs, wavelet,
    ...
    with ProcessPoolExecutor(max_workers=num_batches) as executer:
        futures = [
            executer.submit(
                process_block_2d_wavelet_features_single_block, shm_name, shm_shape, shm_dtype, row_initial, n, batch_index, 
...
def process_block_2d_wavelet_features(shm_name, shm_shape, shm_dtype, row_initial, n, num_batches, output_data_plot_dirs, 
    ...
    """
    print("process_block_2d_wavelet_features")
    shm, full_signal = access_shared_memory(shm_name=shm_name, shm_shape=shm_shape, shm_dtype=shm_dtype)
    # Separate the output for data and plots
    output_data_dir, output_plot_dir = output_data_plot_dirs
    ...
    for batch in range (0, num_batches):
        ...
        for signal_index in range(start, num_pages, n):
            ...
            # Ensure we don't go out of bounds
            signal = full_signal[p_start:(p_end+1), t_start:(t_end+1)]
            print(f"curr signal dims: {signal.shape}")
            print(f"Processing wavelet for block [{t_start}, {t_end}] X [{p_start}, {p_end}]")
            # Perform 2D wavelet transform on the block
            coeffs = process_wavelet_2d(signal, wavelet=wavelet, level=level, is_complex=is_complex, is_mag=True)
...
def process_block_2d_wavelet_decay_features_single_batch(shm_name, shm_shape, shm_dtype, row_initial, n, batch_index, 
                                                         t_start_idx_flt, t_end_idx__flt, num_pages, output_data_plot_dirs, wavelet, 
    ...
    wvlt_zip = wavelet_batch_data_dir + ".zip"

    if(os.path.exists(wvlt_zip)):
        print(f"{wvlt_zip} already exists!!")
        time.sleep(2)
        return
    ...
    for page_idx in range(start, num_pages, n):
        ...
        print("effective_p_start:effective_p_end, t_start_idx:t_end_idx", effective_p_start, ":", 
              effective_p_end, ", ", t_start_idx, ":", t_end_idx, "block size: ", block.shape, " N:", n)
        ...
        file_dir = os.path.join(features_dir, "features.h5")
        ...
        if is_visualize:
            ...
            create_scalogram_1d(coeffs, level, effective_p_start, signal_plot_dir)
    ...
    zip_output_path = os.path.join(wavelet_root_data_dir, f'batch_{batch_index}')

                                   
    # Create a zip archive from the directory
    shutil.make_archive(zip_output_path, 'zip', zip_output_path)

    # Remove the original directory
    subprocess.run(['rm', '-rf', zip_output_path], check=True)
    
    shm.close()


def process_block_2d_wavelet_features_overlap_parallel(shm_name, shm_shape, shm_dtype, row_initial, n, num_batches, output_data_plot_dirs, 
                                                       wavelet, level, is_visualize=False, is_complex=False, page_overlap_percentage=0.5,
                                                       page_decay_rate=0.1, batch_overlap_percentage=0.5):
    
    print("process_block_2d_wavelet_features_overlap_parallel")
    ...
    with ProcessPoolExecutor(max_workers=num_batches) as executer:
        futures = [
            executer.submit(
                process_block_2d_wavelet_decay_features_single_batch, shm_name, shm_shape, shm_dtype, row_initial, n, batch_index, 
                t_start_index, t_end_index, num_pages, output_data_plot_dirs, wavelet, level, is_visualize, is_complex, 
...
# Single block decay
def process_block_2d_wavelet_features_overlap(shm_name, shm_shape, shm_dtype, row_initial, n, num_batches, output_data_plot_dirs, wavelet,
...
def process_2d_scattering_wavelets_with_scalogram(__npy_only, row_initial, shm_name, shm_shape, shm_dtype, n, t, output_data_plot_dirs, is_complex, is_visualize, wavelet, J=4, L=8):
    """
...
def process_2d_scattering_parallel_overlap_single(shm_name, shm_shape, shm_dtype, row_initial, n, batch_index, t_start_idx_flt, t_end_idx__flt, num_pages, output_data_plot_dirs, wavelet, 
...
def process_2d_scattering_parallel_overlap(__npy_only, is_visualize, row_initial, shm_name, shm_shape, shm_dtype, N, num_batches,
    ...
    with ProcessPoolExecutor(max_workers=num_batches) as executer:
        futures = [
            executer.submit(
                process_2d_scattering_parallel_overlap_single, shm_name, shm_shape, shm_dtype, row_initial, N, batch_index, t_start_index, t_end_index, num_pages, output_data_plot_dirs, wavelet, 
...
def process_2d_scattering_wavelets_with_scalogram_overlap(__npy_only, is_visualize, row_initial, shm_name, shm_shape, 
                                                            shm_dtype, N, num_batches, output_data_plot_dirs, is_complex, wavelet,
                                                            J=4, L=8, page_overlap_percentage=0.5, page_decay_rate=0.1, 
                                                           batch_overlap_percentage=0.5):
    """
    Processes blocks of the combined data using 2D Wavelet Scattering, visualizes and saves the coefficients, and generates scalograms.
...
def process_2d_parallel_scattering_wavelets_with_scalogram(__npy_only, row_initial, shm_name, shm_shape, shm_dtype, n, t, 
                                                           output_data_plot_dirs, is_complex, is_visualize, wavelet, K=8, J=4, L=8):
    """
    Processes blocks of the combined data using 2D Wavelet Scattering, visualizes and saves the coefficients, and generates scalograms.
...
def main():
    ...
    if args.analysis == 'wavelet1d_batches':
        process_1d_wavelet_time_segment(args.batches, row_initial, shm_name, shm_shape, shm_dtype, output_data_plot_dirs, wavelet='db4', level=2, is_complex=True)
    elif args.analysis == 'wavelet1d':
        process_1d_wavelet(row_initial, shm_name, shm_shape, shm_dtype, output_data_plot_dirs, wavelet='db4', level=2, is_complex=True)
        ...
        if args.exp_decay:
            if args.scattering:
                if args.parallel:
                    ...
                    process_2d_scattering_parallel_overlap(__npy_only, visualize, row_initial, shm_name, shm_shape, shm_dtype, n, t, output_data_plot_dirs, args.is_complex, 'db4', J=4, L=8)

                else:
                    process_2d_scattering_wavelets_with_scalogram_overlap(__npy_only, visualize, row_initial, shm_name, shm_shape, 
                                                                          shm_dtype, n//4, t, output_data_plot_dirs, args.is_complex, 
                                                                          wavelet='db4', J=4, L=8, page_overlap_percentage=0.5, 
                                                                          page_decay_rate=0.1, batch_overlap_percentage=0.5)
            else:
                if args.parallel:
                    process_block_2d_wavelet_features_overlap_parallel(shm_name, shm_shape, shm_dtype, row_initial, n, t, 
                                                                       output_data_plot_dirs, args.wavelet, level=3, is_visualize=False, 
                                                                       is_complex=False) # TODO: Make is visualize TRUE
                else:
                    process_block_2d_wavelet_features_overlap(shm_name, shm_shape, shm_dtype, row_initial, n, t, output_data_plot_dirs, 
            ...
            if args.scattering:
                if args.parallel:
                    num_of_threads = args.num_threads
                    process_2d_parallel_scattering_wavelets_with_scalogram(__npy_only, row_initial, shm_name, shm_shape, shm_dtype, n, t, output_data_plot_dirs, args.is_complex, visualize, 'db4', num_of_threads, J=4, L=8)
                else:
                    process_2d_scattering_wavelets_with_scalogram(__npy_only, row_initial, shm_name, shm_shape, shm_dtype, n, t, output_data_plot_dirs, args.is_complex, visualize, wavelet='db4', J=4, L=8)
            else:
                if args.parallel:
                    process_block_2d_wavelet_features_parallel(shm_name, shm_shape, shm_dtype, row_initial, n, t, output_data_plot_dirs, 
                                      args.wavelet, level=3, is_visualize=False, is_complex=False)
                else:
                    process_block_2d_wavelet_features(shm_name, shm_shape, shm_dtype, row_initial, n, t, output_data_plot_dirs, 
                                      args.wavelet, level=3, is_visualize=False, is_complex=False)

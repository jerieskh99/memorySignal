...
DWARFS = [
 {"id": "D1", "name": "Dense Linear Algebra", "origin": "Colella-7", "vis": "Visible", "maps": "KERNEL",
  ...
  "workloads": [
    ...
    ("kernel_lu_v2", "planned", "KERNEL family: in-place LU factorisation (-> kernel/kernel_lu_v2.c)", "Visible (shrinking trailing-submatrix front)", "Linear solve: SPICE circuit sim, finite-element, optimisation (LAPACK dgetrf)"),
    ("kernel_qr_v2", "planned", "KERNEL family: Gram-Schmidt / QR orthogonalisation (-> kernel/kernel_qr_v2.c)", "Visible (growing orthogonalised-column front)", "Least-squares regression; GMRES/Arnoldi eigensolvers"),
    ("kernel_attention_v2", "planned", "KERNEL family: scaled dot-product attention QK^T/softmax/*V (-> kernel/kernel_attention_v2.c)", "Visible (transformer core; ~GEMM + row-softmax)", "Every transformer / LLM (the per-token core)"),
    ("kernel_conv_v2", "planned", "KERNEL family: 2D convolution / CNN layer, sliding-window MAC (-> kernel/kernel_conv_v2.c)", "Visible (overlapping-window feature-map rewrite)", "CNNs: image & video vision models"),
 ...
 {"id": "D2", "name": "Sparse Linear Algebra", "origin": "Colella-7", "vis": "Quiet", "maps": "CPU/IDLE control",
  "what": "Indirect gather/scatter via index arrays; irregular reads, small output write. Heavily used "
          "(PageRank, Conjugate Gradient) but near-invisible to the write-signal -- the 'important but "
          "invisible' cluster: gather-dominated with tiny writes.",
  "example": "SpMV, PageRank, Conjugate Gradient, sparse triangular solve, SpGEMM",
  "workloads": [
 ...
 {"id": "D3", "name": "Spectral Methods", "origin": "Colella-7", "vis": "Visible++", "maps": "KERNEL",
  ...
  "workloads": [
    ...
    ("kernel_dct_v2", "planned", "KERNEL family: blocked 8x8 discrete cosine transform (-> kernel/kernel_dct_v2.c)", "Visible (many small blocks; real content)", "JPEG / MPEG / H.264 image & video compression"),
    ("kernel_dwt_v2", "planned", "KERNEL family: discrete wavelet transform, filter+downsample pyramid (-> kernel/kernel_dwt_v2.c)", "Visible (halving multi-resolution pyramid)", "JPEG2000, denoising, audio/image compression"),
    ("kernel_fft2d_v2", "planned", "KERNEL family: 2D FFT (row FFTs, transpose, column FFTs) (-> kernel/kernel_fft2d_v2.c)", "Visible++ (transpose scatter + two-direction passes)", "Image/optics spectral filtering, turbulence DNS, crystallography"),
 ...
 {"id": "D4", "name": "N-Body Methods", "origin": "Colella-7", "vis": "Visible", "maps": "KERNEL",
  ...
  "workloads": [
    ...
    ("kernel_barnes_hut_v2", "planned", "KERNEL family: quadtree build + traversal, O(n log n) (-> kernel/kernel_barnes_hut_v2.c)", "Visible/irregular (tree nodes rebuilt each step + particle rewrite)", "Cosmology (GADGET), galaxy formation"),
    ("kernel_md_lj_v2", "planned", "KERNEL family: Lennard-Jones MD with cell lists (-> kernel/kernel_md_lj_v2.c)", "Visible (periodic cell-list rebuild + position/velocity rewrite)", "GROMACS / NAMD / AMBER: drug discovery, protein folding, materials"),
    ("kernel_pic_v2", "planned", "KERNEL family: particle-in-cell (scatter to grid, field solve, gather) (-> kernel/kernel_pic_v2.c)", "Visible (particle->grid scatter-deposit + grid rewrite)", "Plasma physics, accelerators, semiconductor device sim"),
    ("kernel_fmm_v2", "planned", "KERNEL family: fast multipole (multipole/local expansion arrays on a tree) (-> kernel/kernel_fmm_v2.c)", "Visible (expansion-coefficient arrays + particle rewrite)", "Electrostatics, acoustics, fast O(n) far-field"),
    ("kernel_sph_v2", "planned", "KERNEL family: smoothed-particle hydrodynamics (density/pressure fields) (-> kernel/kernel_sph_v2.c)", "Visible (extra per-particle fields + particle rewrite)", "Fluid sim, film VFX (water/lava), astrophysics"),
...
FAMILIES = [
 {"id": "S1", "name": "IDLE (+ CPU boundary)", "sig": "near-zero writes",
  ...
  "workloads": [
    ...
    ("kernel_spmv_v2", "Sparse Linear Algebra", "SpMV quiet control: gather-dominated, read-only structure -> near-idle (kernel/kernel_spmv_v2.c)"),
  ]},
 ...
 {"id": "S10", "name": "KERNEL (compute motifs)", "sig": "structured-compute writes",
  ...
  "workloads": [
    ("kernel_stencil_jacobi_v2", "Structured Grids", "2D 5-point Jacobi; full-grid rewrite, double-buffer (kernel/kernel_stencil_jacobi_v2.c)"),
    ("kernel_stencil_seidel_v2", "Structured Grids", "Gauss-Seidel red-black, in-place; checkerboard writes (kernel/kernel_stencil_seidel_v2.c)"),
    ("kernel_multigrid_v2", "Structured Grids", "geometric multigrid V-cycle; multi-scale, time-varying footprint (kernel/kernel_multigrid_v2.c)"),
    ("kernel_fft_v2", "Spectral Methods", "in-place radix-2 FFT; stage-varying-stride butterfly + bit-reversal scatter (kernel/kernel_fft_v2.c)"),
    ("kernel_gemm_v2", "Dense Linear Algebra", "blocked dense matmul C=A*B; large output-C rewrite (kernel/kernel_gemm_v2.c)"),
    ("kernel_nbody_v2", "N-Body Methods", "2D particle sim; four compact arrays rewritten per step, smooth evolution (kernel/kernel_nbody_v2.c)"),
    ("kernel_dp_v2", "Dynamic Programming", "edit-distance DP table fill; row-major wavefront (kernel/kernel_dp_v2.c)"),
    ("kernel_hmm_v2", "Graphical Models", "scaled HMM forward; probability-trellis column fill + dense transition matvec (kernel/kernel_hmm_v2.c)"),
    ("kernel_lu_v2", "Dense Linear Algebra", "in-place LU factorisation; shrinking trailing-submatrix front (kernel/kernel_lu_v2.c)"),
    ("kernel_qr_v2", "Dense Linear Algebra", "modified Gram-Schmidt / QR; growing orthogonalised-column front (kernel/kernel_qr_v2.c)"),
    ("kernel_attention_v2", "Dense Linear Algebra", "scaled dot-product attention (QK^T, row softmax, *V); transformer core (kernel/kernel_attention_v2.c)"),
    ("kernel_conv_v2", "Dense Linear Algebra", "2D convolution / CNN layer; overlapping-window MAC, feature-map rewrite (kernel/kernel_conv_v2.c)"),
    ("kernel_ntt_v2", "Spectral Methods", "multi-limb number-theoretic transform (modular butterfly); CKKS/lattice core, no crypto (kernel/kernel_ntt_v2.c)"),
    ("kernel_dct_v2", "Spectral Methods", "blocked 8x8 2D DCT-II (JPEG transform); many small block rewrites (kernel/kernel_dct_v2.c)"),
    ("kernel_dwt_v2", "Spectral Methods", "multi-level 2D Haar wavelet; shrinking multi-resolution pyramid (kernel/kernel_dwt_v2.c)"),
    ("kernel_fft2d_v2", "Spectral Methods", "2D FFT (row FFTs, transpose, column FFTs); transpose scatter (kernel/kernel_fft2d_v2.c)"),
    ("kernel_barnes_hut_v2", "N-Body Methods", "Barnes-Hut quadtree N-body; tree rebuilt each step + particle integrate (kernel/kernel_barnes_hut_v2.c)"),
    ("kernel_md_lj_v2", "N-Body Methods", "Lennard-Jones molecular dynamics; cell list rebuilt each step + velocity-Verlet integrate (kernel/kernel_md_lj_v2.c)"),
    ("kernel_pic_v2", "N-Body Methods", "electrostatic particle-in-cell; CIC scatter/gather + Jacobi Poisson solve on a grid (kernel/kernel_pic_v2.c)"),
    ("kernel_fmm_v2", "N-Body Methods", "single-level fast multipole; per-box complex expansion coefficients + far eval (kernel/kernel_fmm_v2.c)"),
    ("kernel_sph_v2", "N-Body Methods", "smoothed-particle hydrodynamics; two-pass neighbour sum with per-particle density/pressure fields (kernel/kernel_sph_v2.c)"),
    ("kernel_lbm_v2", "Structured Grids", "Lattice-Boltzmann D2Q9; 9 distribution arrays streamed + BGK collide each step (kernel/kernel_lbm_v2.c)"),
    ("kernel_fdtd_v2", "Structured Grids", "2D FDTD electromagnetics; coupled E/H field grids in Yee leapfrog (kernel/kernel_fdtd_v2.c)"),
    ("kernel_floyd_v2", "Dynamic Programming", "Floyd-Warshall all-pairs shortest paths; whole matrix relaxed n times (kernel/kernel_floyd_v2.c)"),
    ("kernel_matrixchain_v2", "Dynamic Programming", "matrix-chain optimal parenthesisation; anti-diagonal fill, O(n^3) (kernel/kernel_matrixchain_v2.c)"),
    ("kernel_knapsack_v2", "Dynamic Programming", "0/1 knapsack, space-optimised 1D rolling capacity array (kernel/kernel_knapsack_v2.c)"),
    ("kernel_smithwaterman_v2", "Dynamic Programming", "Smith-Waterman local alignment; wavefront fill + traceback (kernel/kernel_smithwaterman_v2.c)"),
    ("kernel_beliefprop_v2", "Graphical Models", "loopy sum-product belief propagation on a grid MRF; iterated message arrays (kernel/kernel_beliefprop_v2.c)"),
    ("kernel_kalman_v2", "Graphical Models", "ensemble of Kalman filters; small dense covariance matrices updated per step (kernel/kernel_kalman_v2.c)"),
    ("kernel_gibbs_v2", "Graphical Models", "Gibbs sampling on a Potts/Ising grid; stochastic per-cell resample sweep (kernel/kernel_gibbs_v2.c)"),
    ("kernel_ldpc_v2", "Graphical Models", "LDPC min-sum decoder; bipartite Tanner-graph message passing (kernel/kernel_ldpc_v2.c)"),
  ]},
]

...
that matrix costs on the order of 60\,GiB for a single long cell and approaches
5\,TiB for a 90-cell campaign --- more than the host could hold.
...
asynchronous helper, appends it to a trajectory, and deletes the older dump, so
the persistent footprint of a 90-cell campaign is about 4.7\,MiB --- a reduction
on the order of a million-fold --- while transient disk never exceeds two or
three gigabytes. The compute drops in step: a memory-mapped byte difference takes
300--500\,ms against 1.5--3\,s for the per-page Rust kernel.

Capturing the v3 dataset taught us something we did not design for, and it is the
first honest finding. We requested a 500\,ms sampling interval, but the measured
cadence was 15--21 seconds per snapshot: a 120-second cell yields 7 snapshots, a
300-second cell yields 20, and a 600-second cell yields 28. The estimated
VM-pause fraction is 0.96--0.97, and \texttt{pmemsave} itself runs in about
0.75\,s, so the cadence is set almost entirely by the suspend/snapshot/resume
cycle and the helper's read-back, not by the interval we asked for. The practical
consequence reframes the entire ``how often should we sample'' question: below
this cadence floor, the sampling interval is inert. The right question is not
how often to sample but how cheaply one can snapshot.
...
from 4.79 to 6.35\,dB, all above the 4.5\,dB bar --- yet every one \emph{fails}
...
majority baseline. The genuine, leakage-free family signal in a coarse scalar is
real but weak. We also retract the first draft's stated mechanism: cepstral SNR
...
the shape of that boundary. The cadence floor tells us the cost model, not the
interval, governs temporal resolution. The coverage failure tells us the phasic

...
We investigate whether the behaviour of a workload running inside a virtual machine
can be recognised from a single host-side memory signal, observed black-box and without
any in-guest agent. The signal is the \emph{Active Page Fraction} (APF) -- the fraction
of guest pages that change between consecutive memory snapshots -- augmented with a
per-page \emph{bit-flip magnitude} computed from the same snapshots, which exposes
write-intensive behaviour that the change-or-no-change fraction alone cannot register;
the method consequently requires no instrumentation beyond periodic memory capture.

Workload identity is already recoverable from this signal when a family member has been
observed during training, reaching near-ceiling accuracy ($\approx 0.97$) under the
optimistic leave-one-replicate-out protocol. The open problem, and the subject of this
paper, is the harder regime: generalisation to \emph{unseen} members of a known family,
and recognition of \emph{novel} families never observed during training.

Our central contribution is to cast workload-behaviour identification as \emph{open-set}
family recognition over a host-side memory signal: we model the \emph{normal behaviour}
of each known workload family and, at inference, either assign a trace to a family or
declare it novel. Threat detection reduces to a corollary under this formulation,
accordingly, since a trace assigned to a threat-associated family is flagged without a
dedicated detector.

We further treat family \emph{cohesion} -- whether the members of a family share a memory
signature -- not as an assumption but as a measured quantity, thereby rendering both
outcomes, a coherent family or a taxonomy the signal does not respect, reportable findings.

% Prior-framing contribution register (Plan 05/06 leakage / characterization result);
% retained for reference, to be reconciled with the open-set recogniser framing above.
\begin{itemize}
  ...
  \item \textbf{Contributions}:
    \begin{itemize}
      ...
      \item C6: an interpretable normal-behavior baseline -- a per-behavior
            fingerprint and a normal envelope whose ``outside the band'' rule
            matches the ML one-class detector (0.95 vs.\ 0.93), plus a masquerade
            map naming the normal behavior each threat impersonates -- recasting
            normal behavior from anomaly backdrop into a characterized deliverable.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{A second channel disambiguates but does not detect (Plan 06).} The
        memory-aliased pair -- \texttt{slowburn} (threat) vs.\ \texttt{pagefault}
        (benign), indistinguishable in APF -- is cleanly separated by a
        per-snapshot disk-I/O channel (mean write rate $\sim$0.20 vs.\
        0.002\,MB/s, $\sim$100$\times$). But disk I/O is itself a behavioral axis,
        not a threat marker: the heaviest writer is \emph{benign}
        (\texttt{mmap\_traversal}, 140\,MB/s) and a threat writes essentially
        nothing (\texttt{batched}, memory-capped). Two orthogonal channels buy
        behavioral \emph{disambiguation} of workloads that alias in one channel,
        \emph{not} detection -- reinforcing that no single coarse channel separates
        threat from benign, and that the contribution is characterization. On the
        full 66-cell 2-channel run this splits cleanly: adding disk I/O \emph{helps}
        identification (11-way instance 0.924$\to$0.939; family seen-before
        0.955$\to$0.985; family on an \emph{unseen} workload 0.348$\to$0.455,
        crossing the 0.364 baseline that the memory channel alone could not) and
        \emph{hurts} threat/benign (leave-one-workload-out 0.742$\to$0.636) -- the
        second channel is a characterization instrument, not a detector.
...
\begin{itemize}
  ...
  \item Normal behavior is itself a deliverable: a per-behavior fingerprint and a
        normal envelope yield an interpretable detector on par with the ML
        one-class (0.95 vs.\ 0.93), and a masquerade map names the normal behavior
        each threat impersonates -- a characterization, not merely an anomaly flag.

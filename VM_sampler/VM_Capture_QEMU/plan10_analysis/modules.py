    ...
    M = [
        ...
        mod("window", "divide", "Window time", "W", "slide a window over snapshots: W frames, hop H",
            ...
            params=[num("w", "W_t (frames)", 8), num("h", "H_t (hop)", 4),
                    ...
                    sel("page_mode", "page axis, when the input still has one", [["active", "active: only pages that ever change"], ["all", "all: every page of the dump"]], "active"),
                    num("max_mb", "memory budget for a page-resolution tile (MB)", 256),
                    num("min_changes", "keep pages changing at least this often (page axis; a phase lens needs more than 1)", 1)]),
        ...
        mod("msc", "lens", "MSC", "g",
            "magnitude-squared coherence between adjacent internal windows of the same signal: how much its spectrum repeats over time", spectral=True,
            inputs=[inp("in", ["tiles"])], outputs=[outp("out", "features")],
            params=[num("iw", "internal window", 128), num("ih", "internal step", 64),
                    sel("method", "method", [["welch", "welch: average the spectra, then the ratio"],
                                             ["legacy_adjacent", "legacy: ratio per pair (degenerate, see the flag)"]], "welch"),
                    sel("detrend", "detrend (without it DC owns the coherence)", [["mean", "subtract each segment's mean"], ["none", "raw segments"]], "mean")],
            flags=["msc_single_segment"], impl="runner/stages.py; legacy path reuses magnitude_squared_coherence.py"),
        ...
        mod("baseline", "reference", "Baseline", "b",
            "a reference to compare against: a PLV phase baseline fitted on one clean run (tiles in), or the benign p5-p95 envelope over a set of recordings (features in)",
            inputs=[inp("in", ["tiles", "features"], lab="clean run / benign features")], outputs=[outp("out", "reference")],
            params=[sel("mode", "mode", [["cell", "cell: PLV phase baseline, one clean run, needs complex tiles"],
                                         ["benign", "benign: p5-p95 envelope per feature, needs features"]], "cell"),
                    {"k": "recording", "kind": "select", "lab": "clean recording for cell mode (empty = first selected)", "opts": [], "default": ""},
                    {"k": "recordings", "kind": "cells", "lab": "benign set for benign mode (none = every recording reaching this node)", "default": []},
                    num("q_lo", "lower percentile", 5.0), num("q_hi", "upper percentile", 95.0)],
            impl="coherence_temp_spec_stability/plv_calcolator.py; plan05_campaign/normal_profile.py"),
        mod("deviation", "lens", "Deviation", "d",
            "how far each tile falls outside the benign envelope: normal_profile.py's count of features outside the band, plus the normalised distance past its edge",
            inputs=[inp("in", ["features"]), inp("ref", ["reference"], lab="envelope")], outputs=[outp("out", "features")],
            impl="plan05_campaign/normal_profile.py deviation()"),
